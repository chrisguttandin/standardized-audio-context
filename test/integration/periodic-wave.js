...
describe('PeriodicWave', { skip: typeof window === 'undefined' }, () => {
    describe.for(Object.entries(testCases))('with the %s', ([description, { createContext, createPeriodicWave }]) => {
        let context;
                ...
                describe('with invalid options', () => {
                    describe('with an imag and real property with a different length', () => {
                        it('should throw an IndexSizeError', () => {
                            expect(() => createPeriodicWave(context, { imag: [1, 2, 3], real: [1, 2, 3, 4, 5] }))
                    ...
                    describe('with an imag and real property with a length of 1', () => {
                        it('should throw an IndexSizeError', () => {
                            expect(() => createPeriodicWave(context, { imag: [1], real: [1] }))
                                .to.throw(DOMException)
                                .to.include({ code: 1, name: 'IndexSizeError' });
                        });
                    });
                    ...
                    if (description.includes('constructor')) {
                        describe('with an imag property of only one value', () => {
                            it('should throw an IndexSizeError', () => {
                                expect(() => createPeriodicWave(context, { imag: [1] }))
                                    .to.throw(DOMException)
                                    .to.include({ code: 1, name: 'IndexSizeError' });
                            });
                        });
                        ...
                        describe('with a real property of only one value', () => {
                            it('should throw an IndexSizeError', () => {
                                expect(() => createPeriodicWave(context, { real: [1] }))
                                    .to.throw(DOMException)
                                    .to.include({ code: 1, name: 'IndexSizeError' });
                            });
                        });
